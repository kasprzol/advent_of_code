...
def simulate_sand_2(cave, cave_min_x, cave_max_x, cave_max_y):
    print_cave(cave, cave_max_y)
    is_current_sand_moving = False
    reached_start = False
    active_sand = None
    ...
    while not reached_start:
        ...
        while cave[active_sand.x][active_sand.y + 1] == EMPTY and active_sand.y + 1 < floor:
            active_sand = Point(active_sand.x, active_sand.y + 1)
            if active_sand.y >= floor:
                is_current_sand_moving = False
                break

        # can move left-down?
        if active_sand.y + 1 < floor and cave[active_sand.x - 1][active_sand.y + 1] == EMPTY:
            active_sand = Point(active_sand.x - 1, active_sand.y + 1)
        # can move right-down?
        elif active_sand.y + 1 < floor and cave[active_sand.x + 1][active_sand.y + 1] == EMPTY:
            active_sand = Point(active_sand.x + 1, active_sand.y + 1)
        else:
            # the sand had rested
            cave[active_sand.x][active_sand.y] = SAND
            ...
            if units_of_sand % 100 == 0:
                print_cave(cave, floor, active_sand)
            if active_sand.x == SAND_START_X and active_sand.y == SAND_START_Y:
                print_cave(cave, floor, active_sand)
                print(units_of_sand)
                return
        ...
        for points in pairwise(segment):
            ...
            if points[0].x == points[1].x:
                ...
                for x in range(min_x, max_x + 1):
                    cave[x][points[0].y] = ROCK
    cave[500][0] = SAND_START
    simulate_sand_2(cave, cave_min_x, cave_max_x, cave_max_y)
